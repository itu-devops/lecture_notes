...
==== BSc
  ...
  * Group c _Group A_ `aleso`, `jkof`, `lukb`, `mfau`, `sals`
  * Group f _benis_ `momj`, `rhen`, `seho`, `jacj`, `emio`
  * Group g _Call Of Duty BlackOps_ `lakl`, `frot`, `adbe`, `cafm`, `anlf`
  * Group h _devHops_ `smdi`, `luvu`, `ahto`, `lann`, `siam`
  * Group j _Root_ `milb`, `deyi`, `deni`, `antni`, `alja`
  * Group l _iBois_ `jacmo`, `askw`, `cger`, `ladr`, `tovs`, `alwe`
  * Group o _DevOpps_ `osbr`, `jlhj`, `hjel`, `anpf`, `chkg`
  * Group p _TBD_ `kong`, `clyt`, `eyey`, `csbe`

==== MSc

  * Group b _Group Bee_ `sarw`, `fadd`, `dibo`, `vise`, `lucn`
  * Group d _DevelOps_ `boer`, `tesp`, `asno`, `calm`, `jtth`
  * Group e _big E_ `npal`, `osha`, `jubr`, `oeje`, `jsow`
